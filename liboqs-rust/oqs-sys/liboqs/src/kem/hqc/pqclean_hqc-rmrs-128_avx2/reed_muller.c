...
void PQCLEAN_HQCRMRS128_AVX2_reed_muller_encode_single(uint8_t *cdw, const uint8_t *msg) {
    size_t i = 0;
    // encode first word
    encode(&cdw[16 * i * MULTIPLICITY], msg[i]);
    // copy to other identical codewords
...
void PQCLEAN_HQCRMRS128_AVX2_reed_muller_decode_single(uint8_t *msg, const uint8_t *cdw) {
    __m256i expanded[8];
    __m256i transform[8];
    size_t i = 0;
    // collect the codewords
