...
void OQS_KEM_hqc_192_code_encode(uint8_t *em, const uint8_t *message) {
	PQCLEAN_HQCRMRS192_AVX2_code_encode(em, message);
}

void OQS_KEM_hqc_192_code_decode(uint8_t *m, const uint8_t *em) {
	PQCLEAN_HQCRMRS192_AVX2_code_decode(m, em);
}

void OQS_KEM_hqc_192_reed_muller_encode_single(uint8_t *cdw, const uint8_t *msg) {
	PQCLEAN_HQCRMRS192_AVX2_reed_muller_encode_single(cdw, msg);
}

void OQS_KEM_hqc_192_reed_muller_decode_single(uint8_t *msg, const uint8_t *cdw) {
	PQCLEAN_HQCRMRS192_AVX2_reed_muller_decode_single(msg, cdw);
}


OQS_STATUS OQS_KEM_hqc_192_encaps_chosen_inputs(uint8_t *ciphertext, uint8_t *shared_secret, const uint8_t *public_key, const unsigned char *m, const unsigned char *r1, const unsigned char *r2, unsigned const char *e) {
	return PQCLEAN_HQCRMRS192_AVX2_crypto_kem_enc_chosen_inputs(ciphertext, shared_secret, public_key, m, r1, r2, e);
}

void OQS_KEM_hqc_192_public_key_from_string(uint64_t *h, uint64_t *s, const uint8_t *pk) {
	PQCLEAN_HQCRMRS192_AVX2_hqc_public_key_from_string(h, s, pk);
}

void OQS_KEM_hqc_192_secret_key_from_string(uint64_t *x, uint64_t *y, uint8_t *pk, const uint8_t *sk) {
...
void OQS_KEM_hqc_192_vect_mul(uint64_t *o, const uint8_t *a1, const uint8_t *a2) {
	PQCLEAN_HQCRMRS192_AVX2_vect_mul(o, a1, a2);
}

void OQS_KEM_hqc_192_vect_add(uint64_t *o, const uint64_t *v1, const uint64_t *v2, uint32_t size) {
	PQCLEAN_HQCRMRS192_AVX2_vect_add(o, v1, v2, size);
}

OQS_API uint32_t OQS_KEM_hqc_192_num_seedexpansions(const uint8_t *m) {
	return PQCLEAN_HQCRMRS192_AVX2_hqc_num_seedexpansions(m);
}
