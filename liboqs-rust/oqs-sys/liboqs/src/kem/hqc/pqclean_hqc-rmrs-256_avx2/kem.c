...
int PQCLEAN_HQCRMRS256_AVX2_crypto_kem_enc_chosen_inputs(unsigned char *ct, unsigned char *ss, const unsigned char *pk, const unsigned char *m, const unsigned char *u, const unsigned char *r2, const unsigned char *e) {
    ...
    // Encrypting m
    PQCLEAN_HQCRMRS256_AVX2_hqc_pke_encrypt_chosen_inputs(u, v, m, theta, pk, r2, e);
...
uint32_t PQCLEAN_HQCRMRS256_AVX2_hqc_num_seedexpansions(const uint8_t *m) {
    ...
    aligned_vec_t ve = {0};
    uint64_t *e = ve.arr64;
    expansions += PQCLEAN_HQCRMRS256_AVX2_vect_set_random_fixed_weight(&seedexpander, r1, PARAM_OMEGA_R);
    expansions += PQCLEAN_HQCRMRS256_AVX2_vect_set_random_fixed_weight(&seedexpander, r2, PARAM_OMEGA_R);
    expansions += PQCLEAN_HQCRMRS256_AVX2_vect_set_random_fixed_weight(&seedexpander, e, PARAM_OMEGA_E);
    return expansions;
}
