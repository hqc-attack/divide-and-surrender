...
void HQC_R4_128_AVX2_hqc_pke_keygen(unsigned char* pk, unsigned char* sk) {
    ...
    HQC_R4_128_AVX2_shake_prng(sk_seed, SEED_BYTES);
    HQC_R4_128_AVX2_shake_prng(sigma, VEC_K_SIZE_BYTES);
    HQC_R4_128_AVX2_seedexpander_init(&sk_seedexpander, sk_seed, SEED_BYTES);

    HQC_R4_128_AVX2_shake_prng(pk_seed, SEED_BYTES);
    HQC_R4_128_AVX2_seedexpander_init(&pk_seedexpander, pk_seed, SEED_BYTES);

    // Compute secret key
    HQC_R4_128_AVX2_vect_set_random_fixed_weight(&sk_seedexpander, x_256, PARAM_OMEGA);
    HQC_R4_128_AVX2_vect_set_random_fixed_weight(&sk_seedexpander, y_256, PARAM_OMEGA);

    // Compute public key
    HQC_R4_128_AVX2_vect_set_random(&pk_seedexpander, (uint64_t *) h_256);
    HQC_R4_128_AVX2_vect_mul(tmp_256, y_256, h_256);
    HQC_R4_128_AVX2_vect_add(s, (uint64_t *) x_256, (uint64_t *) tmp_256, VEC_N_256_SIZE_64);

    // Parse keys to string
    HQC_R4_128_AVX2_hqc_public_key_to_string(pk, pk_seed, s);
    HQC_R4_128_AVX2_hqc_secret_key_to_string(sk, sk_seed, sigma, pk);

    #ifdef VERBOSE
        printf("\n\nsk_seed: "); for(int i = 0 ; i < SEED_BYTES ; ++i) printf("%02x", sk_seed[i]);
        printf("\n\nsigma: "); for(int i = 0 ; i < VEC_K_SIZE_BYTES ; ++i) printf("%02x", sigma[i]);
        printf("\n\nx: "); HQC_R4_128_AVX2_vect_print((uint64_t *) x_256, VEC_N_SIZE_BYTES);
        printf("\n\ny: "); HQC_R4_128_AVX2_vect_print((uint64_t *) y_256, VEC_N_SIZE_BYTES);

        printf("\n\npk_seed: "); for(int i = 0 ; i < SEED_BYTES ; ++i) printf("%02x", pk_seed[i]);
        printf("\n\nh: "); HQC_R4_128_AVX2_vect_print((uint64_t *) h_256, VEC_N_SIZE_BYTES);
        printf("\n\ns: "); HQC_R4_128_AVX2_vect_print(s, VEC_N_SIZE_BYTES);
...
void HQC_R4_128_AVX2_hqc_pke_encrypt(uint64_t *u, uint64_t *v, uint64_t *m, unsigned char *theta, const unsigned char *pk) {
    ...
    HQC_R4_128_AVX2_seedexpander_init(&seedexpander, theta, SEED_BYTES);

    // Retrieve h and s from public key
    HQC_R4_128_AVX2_hqc_public_key_from_string((uint64_t *) h_256, (uint64_t *) s_256, pk);

    // Generate r1, r2 and e
    HQC_R4_128_AVX2_vect_set_random_fixed_weight(&seedexpander, r1_256, PARAM_OMEGA_R);
    HQC_R4_128_AVX2_vect_set_random_fixed_weight(&seedexpander, r2_256, PARAM_OMEGA_R);
    HQC_R4_128_AVX2_vect_set_random_fixed_weight(&seedexpander, e_256, PARAM_OMEGA_E);

    // Compute u = r1 + r2.h
    HQC_R4_128_AVX2_vect_mul(tmp1_256, r2_256, h_256);
    HQC_R4_128_AVX2_vect_add(u, (uint64_t *) r1_256, (uint64_t *) tmp1_256, VEC_N_256_SIZE_64);

    // Compute v = m.G by encoding the message
    HQC_R4_128_AVX2_code_encode(v, m);
    HQC_R4_128_AVX2_vect_resize((uint64_t *) tmp2_256, PARAM_N, v, PARAM_N1N2);

    // Compute v = m.G + s.r2 + e
    HQC_R4_128_AVX2_vect_mul(tmp3_256, r2_256, s_256);
    HQC_R4_128_AVX2_vect_add(tmp4, (uint64_t *) e_256, (uint64_t *) tmp3_256, VEC_N_256_SIZE_64);
    HQC_R4_128_AVX2_vect_add((uint64_t *) tmp3_256, (uint64_t *) tmp2_256, tmp4, VEC_N_256_SIZE_64);
    HQC_R4_128_AVX2_vect_resize(v, PARAM_N1N2, (uint64_t *) tmp3_256, PARAM_N);

    #ifdef VERBOSE
        printf("\n\nh: "); HQC_R4_128_AVX2_vect_print((uint64_t *) h_256, VEC_N_SIZE_BYTES);
        printf("\n\ns: "); HQC_R4_128_AVX2_vect_print((uint64_t *) s_256, VEC_N_SIZE_BYTES);
        printf("\n\nr1: "); HQC_R4_128_AVX2_vect_print((uint64_t *) r1_256, VEC_N_SIZE_BYTES);
        printf("\n\nr2: "); HQC_R4_128_AVX2_vect_print((uint64_t *) r2_256, VEC_N_SIZE_BYTES);
        printf("\n\ne: "); HQC_R4_128_AVX2_vect_print((uint64_t *) e_256, VEC_N_SIZE_BYTES);
        printf("\n\ntmp3_256: "); HQC_R4_128_AVX2_vect_print((uint64_t *) tmp3_256, VEC_N_SIZE_BYTES);

        printf("\n\nu: "); HQC_R4_128_AVX2_vect_print(u, VEC_N_SIZE_BYTES);
        printf("\n\nv: "); HQC_R4_128_AVX2_vect_print(v, VEC_N1N2_SIZE_BYTES);
    #endif
}

void HQC_R4_128_AVX2_hqc_pke_encrypt_chosen_inputs(uint64_t *u, uint64_t *v, uint8_t *m, unsigned char *theta, const unsigned char *pk, const unsigned char *r2I, const unsigned char *eI) {
    seedexpander_state seedexpander;
    __m256i h_256[VEC_N_256_SIZE_64 >> 2];
    __m256i s_256[VEC_N_256_SIZE_64 >> 2];
    __m256i r2_256[VEC_N_256_SIZE_64 >> 2];
...
uint8_t HQC_R4_128_AVX2_hqc_pke_decrypt(uint64_t *m, uint8_t *sigma, const __m256i *u_256, const uint64_t *v, const uint8_t *sk) {
    ...
    HQC_R4_128_AVX2_hqc_secret_key_from_string(x_256, y_256, sigma, pk, sk);

    // Compute v - u.y
    HQC_R4_128_AVX2_vect_resize(tmp1, PARAM_N, v, PARAM_N1N2);
    HQC_R4_128_AVX2_vect_mul(tmp3_256, y_256, u_256);
    HQC_R4_128_AVX2_vect_add(tmp2, tmp1, (uint64_t *) tmp3_256, VEC_N_256_SIZE_64);
    ...
    HQC_R4_128_AVX2_code_decode(m, tmp2);
