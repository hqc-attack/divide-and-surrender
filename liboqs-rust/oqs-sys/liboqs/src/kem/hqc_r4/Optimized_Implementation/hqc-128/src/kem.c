 ...
 */
int HQC_R4_128_AVX2_crypto_kem_keypair(unsigned char *pk, unsigned char *sk) {
    #ifdef VERBOSE
        printf("\n\n\n\n### KEYGEN ###");
    #endif

    HQC_R4_128_AVX2_hqc_pke_keygen(pk, sk);
    return 0;
}
 ...
 */
int HQC_R4_128_AVX2_crypto_kem_enc(unsigned char *ct, unsigned char *ss, const unsigned char *pk) {
    #ifdef VERBOSE
        printf("\n\n\n\n### ENCAPS ###");
    #endif
    ...
    // Computing m
    HQC_R4_128_AVX2_vect_set_random_from_prng((uint64_t *)m, VEC_K_SIZE_64);

    // Computing theta
    HQC_R4_128_AVX2_vect_set_random_from_prng(salt, SALT_SIZE_64);
    ...
    HQC_R4_128_AVX2_shake256_512_ds(&shake256state, theta, tmp, VEC_K_SIZE_BYTES + PUBLIC_KEY_BYTES + SALT_SIZE_BYTES, G_FCT_DOMAIN);

    // Encrypting m
    HQC_R4_128_AVX2_hqc_pke_encrypt(u, v, (uint64_t *)m, theta, pk);

    // Computing shared secret
    memcpy(mc, m, VEC_K_SIZE_BYTES);
    memcpy(mc + VEC_K_SIZE_BYTES, u, VEC_N_SIZE_BYTES);
    memcpy(mc + VEC_K_SIZE_BYTES + VEC_N_SIZE_BYTES, v, VEC_N1N2_SIZE_BYTES);
    HQC_R4_128_AVX2_shake256_512_ds(&shake256state, ss, mc, VEC_K_SIZE_BYTES + VEC_N_SIZE_BYTES + VEC_N1N2_SIZE_BYTES, K_FCT_DOMAIN);

    // Computing ciphertext
    HQC_R4_128_AVX2_hqc_ciphertext_to_string(ct, u, v, salt);
...
int HQC_R4_128_AVX2_crypto_kem_enc_chosen_inputs(unsigned char *ct, unsigned char *ss, const unsigned char *pk, const unsigned char *m, const unsigned char *u, const unsigned char *r2, const unsigned char *e, const unsigned char *salt) {
    #ifdef VERBOSE
        printf("\n\n\n\n### ENCAPS ###");
    #endif
    ...
    // Encrypting m
    HQC_R4_128_AVX2_hqc_pke_encrypt_chosen_inputs(u, v, (uint64_t *)m, theta, pk, r2, e);
 ...
 */
int HQC_R4_128_AVX2_crypto_kem_dec(unsigned char *ss, const unsigned char *ct, const unsigned char *sk) {
    #ifdef VERBOSE
        printf("\n\n\n\n### DECAPS ###");
    #endif
    ...
    HQC_R4_128_AVX2_hqc_ciphertext_from_string((uint64_t *) u_256, v, salt, ct);

    // Retrieving pk from sk
    memcpy(pk, sk + SEED_BYTES + VEC_K_SIZE_BYTES, PUBLIC_KEY_BYTES);

    // Decrypting
    result = HQC_R4_128_AVX2_hqc_pke_decrypt((uint64_t *)m, sigma, u_256, v, sk);
    ...
    // Encrypting m'
    HQC_R4_128_AVX2_hqc_pke_encrypt(u2, v2, (uint64_t *)m, theta, pk);

    // Check if c != c'
    result |= HQC_R4_128_AVX2_vect_compare((uint8_t *) u_256, (uint8_t *) u2, VEC_N_SIZE_BYTES);
    result |= HQC_R4_128_AVX2_vect_compare((uint8_t *) v, (uint8_t *) v2, VEC_N1N2_SIZE_BYTES);
    ...
    HQC_R4_128_AVX2_shake256_512_ds(&shake256state, ss, mc, VEC_K_SIZE_BYTES + VEC_N_SIZE_BYTES + VEC_N1N2_SIZE_BYTES, K_FCT_DOMAIN);
        ...
        printf("\n\nciphertext: "); for(int i = 0 ; i < CIPHERTEXT_BYTES ; ++i) printf("%02x", ct[i]);
        printf("\n\nm: "); HQC_R4_128_AVX2_vect_print((uint64_t *)m, VEC_K_SIZE_BYTES);
        printf("\n\ntheta: "); for(int i = 0 ; i < SHAKE256_512_BYTES ; ++i) printf("%02x", theta[i]);
        printf("\n\n\n# Checking Ciphertext- Begin #");
        printf("\n\nu2: "); HQC_R4_128_AVX2_vect_print(u2, VEC_N_SIZE_BYTES);
        printf("\n\nv2: "); HQC_R4_128_AVX2_vect_print(v2, VEC_N1N2_SIZE_BYTES);
...
uint32_t HQC_R4_128_AVX2_division_latency(const uint8_t *m, const uint8_t *pk, const uint8_t *salt) {
    ...
    HQC_R4_128_AVX2_seedexpander_init(&ctx, theta, SEED_BYTES);
    ...
    for (int j = 0; j < 3; ++j) {
        ...
        HQC_R4_128_AVX2_seedexpander(&ctx, (uint8_t *)&rand_u32, 4 * weight);
