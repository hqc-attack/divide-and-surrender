 ...
 */
void HQC_R4_128_AVX2_shake256_512_ds(shake256incctx *state, uint8_t *output, const uint8_t *input, size_t inlen, uint8_t domain) {
    /* Init state */
    HQC_R4_128_AVX2_shake256_inc_init(state);

    /* Absorb input */
    HQC_R4_128_AVX2_shake256_inc_absorb(state, input, inlen);
    ...
    /* Squeeze output */
    HQC_R4_128_AVX2_shake256_inc_squeeze(output, 512/8, state);
}
