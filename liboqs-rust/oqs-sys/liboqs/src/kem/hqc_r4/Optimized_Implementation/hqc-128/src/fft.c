...
static void HQC_R4_128_AVX2_compute_fft_betas(uint16_t *betas);
static void HQC_R4_128_AVX2_compute_subset_sums(uint16_t *subset_sums, const uint16_t *set, uint16_t set_size);
static void HQC_R4_128_AVX2_radix(uint16_t *f0, uint16_t *f1, const uint16_t *f, uint32_t m_f);
static void HQC_R4_128_AVX2_radix_big(uint16_t *f0, uint16_t *f1, const uint16_t *f, uint32_t m_f);
static void HQC_R4_128_AVX2_fft_rec(uint16_t *w, uint16_t *f, size_t f_coeffs, uint8_t m, uint32_t m_f, const uint16_t *betas);
...
static void HQC_R4_128_AVX2_radix(uint16_t *f0, uint16_t *f1, const uint16_t *f, uint32_t m_f) {
    switch (m_f) {
        ...
        default:
            HQC_R4_128_AVX2_radix_big(f0, f1, f, m_f);
            break;
    }
}
...
static void HQC_R4_128_AVX2_radix_big(uint16_t *f0, uint16_t *f1, const uint16_t *f, uint32_t m_f) {
    ...
    HQC_R4_128_AVX2_radix(Q0, Q1, Q, m_f - 1);
    HQC_R4_128_AVX2_radix(R0, R1, R, m_f - 1);
...
static void HQC_R4_128_AVX2_fft_rec(uint16_t *w, uint16_t *f, size_t f_coeffs, uint8_t m, uint32_t m_f, const uint16_t *betas) {
    ...
    if (m_f == 1) {
        for (i = 0; i < m; ++i) {
            tmp[i] = HQC_R4_128_AVX2_gf_mul(betas[i], f[1]);
        }
    ...
    if (betas[m - 1] != 1) {
        ...
        x <<= m_f;
        for (i = 1; i < x; ++i) {
            beta_m_pow = HQC_R4_128_AVX2_gf_mul(beta_m_pow, betas[m - 1]);
            f[i] = HQC_R4_128_AVX2_gf_mul(beta_m_pow, f[i]);
        }
    }

    // Step 3
    HQC_R4_128_AVX2_radix(f0, f1, f, m_f);

    // Step 4: compute gammas and deltas
    for (i = 0; i + 1 < m; ++i) {
        gammas[i] = HQC_R4_128_AVX2_gf_mul(betas[i], HQC_R4_128_AVX2_gf_inverse(betas[m - 1]));
        deltas[i] = HQC_R4_128_AVX2_gf_square(gammas[i]) ^ gammas[i];
    }

    // Compute gammas sums
    HQC_R4_128_AVX2_compute_subset_sums(gammas_sums, gammas, m - 1);

    // Step 5
    HQC_R4_128_AVX2_fft_rec(u, f0, (f_coeffs + 1) / 2, m - 1, m_f - 1, deltas);
    ...
    if (f_coeffs <= 3) { // 3-coefficient polynomial f case: f1 is constant
        ...
        w[k] = u[0] ^ f1[0];
        for (i = 1; i < k; ++i) {
            w[i] = u[i] ^ HQC_R4_128_AVX2_gf_mul(gammas_sums[i], f1[0]);
            w[k + i] = w[i] ^ f1[0];
        }
    } else {
        HQC_R4_128_AVX2_fft_rec(v, f1, f_coeffs / 2, m - 1, m_f - 1, deltas);
        ...
        w[k] ^= u[0];
        for (i = 1; i < k; ++i) {
            w[i] = u[i] ^ HQC_R4_128_AVX2_gf_mul(gammas_sums[i], v[i]);
            w[k + i] ^= w[i];
        }
    }
}
 ...
 */
void HQC_R4_128_AVX2_fft(uint16_t *w, const uint16_t *f, size_t f_coeffs) {
    ...
    HQC_R4_128_AVX2_compute_fft_betas(betas);

    // Step 1: PARAM_FFT > 1, nothing to do

    // Compute gammas sums
    HQC_R4_128_AVX2_compute_subset_sums(betas_sums, betas, PARAM_M - 1);

    // Step 2: beta_m = 1, nothing to do

    // Step 3
    HQC_R4_128_AVX2_radix(f0, f1, f, PARAM_FFT);

    // Step 4: Compute deltas
    for (i = 0; i < PARAM_M - 1; ++i) {
        deltas[i] = HQC_R4_128_AVX2_gf_square(betas[i]) ^ betas[i];
    }

    // Step 5
    HQC_R4_128_AVX2_fft_rec(u, f0, (f_coeffs + 1) / 2, PARAM_M - 1, PARAM_FFT - 1, deltas);
    HQC_R4_128_AVX2_fft_rec(v, f1, f_coeffs / 2, PARAM_M - 1, PARAM_FFT - 1, deltas);
    ...
    // Find other roots
    for (i = 1; i < k; ++i) {
        w[i] = u[i] ^ HQC_R4_128_AVX2_gf_mul(betas_sums[i], v[i]);
        w[k + i] ^= w[i];
    }
}
 ...
 */
void HQC_R4_128_AVX2_fft_retrieve_error_poly(uint8_t *error, const uint16_t *w) {
    uint16_t gammas[PARAM_M - 1] = {0};
    uint16_t gammas_sums[1 << (PARAM_M - 1)] = {0};
    uint16_t k;
    size_t i, index;

    HQC_R4_128_AVX2_compute_fft_betas(gammas);
    HQC_R4_128_AVX2_compute_subset_sums(gammas_sums, gammas, PARAM_M - 1);
