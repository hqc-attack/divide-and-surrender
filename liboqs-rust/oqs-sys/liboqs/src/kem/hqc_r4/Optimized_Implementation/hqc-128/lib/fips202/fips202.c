...
void HQC_R4_128_AVX2_shake128(uint8_t *output, size_t outlen,
    ...
    HQC_R4_128_AVX2_shake128_absorb(&s, input, inlen);
    HQC_R4_128_AVX2_shake128_squeezeblocks(output, nblocks, &s);

    output += nblocks * SHAKE128_RATE;
    outlen -= nblocks * SHAKE128_RATE;

    if (outlen) {
        HQC_R4_128_AVX2_shake128_squeezeblocks(t, 1, &s);
...
void HQC_R4_128_AVX2_shake256(uint8_t *output, size_t outlen,
    ...
    HQC_R4_128_AVX2_shake256_absorb(&s, input, inlen);
    HQC_R4_128_AVX2_shake256_squeezeblocks(output, nblocks, &s);

    output += nblocks * SHAKE256_RATE;
    outlen -= nblocks * SHAKE256_RATE;

    if (outlen) {
        HQC_R4_128_AVX2_shake256_squeezeblocks(t, 1, &s);
        for (size_t i = 0; i < outlen; ++i) {
            output[i] = t[i];
        }
    }
