...
static const __m256i param256[2] = {
    {0x0074009900450059, 0x004B006F007500B0, 0x00E900F200E90049, 0x008B001500D20041},
    {0x0076004300AD0067, 0x006E00AE00D20069, 0x005200E40045004A, 0x0000000100B500FF}
};
...
void HQC_R4_128_AVX2_reed_solomon_encode(uint64_t *cdw, const uint64_t *msg) {
    ...
    for (int16_t i = PARAM_K - 1; i >= 0; --i) {
        ...
        tmp256[0] = HQC_R4_128_AVX2_gf_mul_vect(gate256, param256[0]);
        tmp256[1] = HQC_R4_128_AVX2_gf_mul_vect(gate256, param256[1]);
...
void compute_syndromes(__m256i *syndromes256, uint8_t *cdw) {
    ...
    for (size_t i = 0; i < PARAM_N1 - 1; ++i) {
        syndromes256[0] ^= HQC_R4_128_AVX2_gf_mul_vect(_mm256_set1_epi16(cdw[i + 1]), alpha_ij256_1[i]);
    }

    __m256i last_syndromes256 = _mm256_set1_epi16(cdw[0]);

    for (size_t i = 0; i < PARAM_N1 - 1; ++i) {
        last_syndromes256 ^= HQC_R4_128_AVX2_gf_mul_vect(_mm256_set1_epi16(cdw[i + 1]), alpha_ij256_2[i]);
    }
...
static uint16_t compute_elp(uint16_t *sigma, const uint16_t *syndromes) {
    ...
    for (mu = 0; (mu < (2 * PARAM_DELTA)); ++mu) {
        ...
        dd = HQC_R4_128_AVX2_gf_mul(d, HQC_R4_128_AVX2_gf_inverse(d_p));

        for (i = 1; (i <= mu + 1) && (i <= PARAM_DELTA); ++i) {
            sigma[i] ^= HQC_R4_128_AVX2_gf_mul(dd, X_sigma_p[i]);
        }
        ...
        for (i = 1; (i <= mu + 1) && (i <= PARAM_DELTA); ++i) {
            d ^= HQC_R4_128_AVX2_gf_mul(sigma[i], syndromes[mu + 1 - i]);
        }
    }
...
static void compute_roots(uint8_t *error, uint16_t *sigma) {
    ...
    HQC_R4_128_AVX2_fft(w, sigma, PARAM_DELTA + 1);
    HQC_R4_128_AVX2_fft_retrieve_error_poly(error, w);
}
...
static void compute_z_poly(uint16_t *z, const uint16_t *sigma, uint16_t degree, const uint16_t *syndromes) {
    ...
    for (i = 2; i <= PARAM_DELTA; ++i) {
        ...
        for (j = 1; j < i; ++j) {
            z[i] ^= mask & HQC_R4_128_AVX2_gf_mul(sigma[j], syndromes[i - j - 1]);
        }
    }
}
...
static void compute_error_values(uint16_t *error_values, const uint16_t *z, const uint8_t *error) {
    ...
    for (size_t i = 0; i < PARAM_DELTA; ++i) {
        tmp1 = 1;
        tmp2 = 1;
        inverse = HQC_R4_128_AVX2_gf_inverse(beta_j[i]);
        inverse_power_j = 1;

        for (size_t j = 1; j <= PARAM_DELTA; ++j) {
            inverse_power_j = HQC_R4_128_AVX2_gf_mul(inverse_power_j, inverse);
            tmp1 ^= HQC_R4_128_AVX2_gf_mul(inverse_power_j, z[j]);
        }
        for (size_t k = 1; k < PARAM_DELTA; ++k) {
            tmp2 = HQC_R4_128_AVX2_gf_mul(tmp2, (1 ^ HQC_R4_128_AVX2_gf_mul(inverse, beta_j[(i + k) % PARAM_DELTA])));
        }
        mask1 = (uint16_t) (((int16_t) i - delta_real_value) >> 15); // i < delta_real_value
        e_j[i] = mask1 & HQC_R4_128_AVX2_gf_mul(tmp1, HQC_R4_128_AVX2_gf_inverse(tmp2));
    }
