...
static void HQC_R4_128_AVX2_encode(codeword *word, int32_t message);
static void HQC_R4_128_AVX2_expand_and_sum(expandedCodeword *dst, codeword src[]);
static void HQC_R4_128_AVX2_hadamard(expandedCodeword *src, expandedCodeword *dst);
static int32_t HQC_R4_128_AVX2_find_peaks(expandedCodeword *transform);
 ...
 */
static inline void HQC_R4_128_AVX2_encode(codeword *word, int32_t message) {
    int32_t first_word;
...
void HQC_R4_128_AVX2_reed_muller_encode_single(uint8_t *cdw, const uint8_t *msg) {
    size_t i = 0;
    // encode first word
    HQC_R4_128_AVX2_encode(&cdw[16 * i * MULTIPLICITY], msg[i]);
 ...
 */
static inline void HQC_R4_128_AVX2_expand_and_sum(expandedCodeword *dst, codeword src[]) {
 ...
 * @param[out] dst Structure that contain the expanded codeword
 */
static inline void HQC_R4_128_AVX2_hadamard(expandedCodeword *src, expandedCodeword *dst) {
 ...
 */
static inline int32_t HQC_R4_128_AVX2_find_peaks(expandedCodeword *transform) {
 ...
 */
void HQC_R4_128_AVX2_reed_muller_encode(uint64_t *cdw, const uint64_t *msg) {
    ...
    for (size_t i = 0; i < VEC_N1_SIZE_BYTES; i++) {
        ...
        // encode first word
        HQC_R4_128_AVX2_encode(&codeArray[pos], message_array[i]);
        // copy to other identical codewords
        for (size_t copy = 1; copy < MULTIPLICITY; copy++) {
 ...
 */
void HQC_R4_128_AVX2_reed_muller_decode(uint64_t *msg, const uint64_t *cdw) {
    uint8_t *message_array = (uint8_t *) msg;
    codeword *codeArray = (codeword *) cdw;
    expandedCodeword expanded;
    for (size_t i = 0; i < VEC_N1_SIZE_BYTES; i++) {
        // collect the codewords
        HQC_R4_128_AVX2_expand_and_sum(&expanded, &codeArray[i * MULTIPLICITY]);
        // apply hadamard transform
        expandedCodeword transform;
        HQC_R4_128_AVX2_hadamard(&expanded, &transform);
        // fix the first entry to get the half Hadamard transform
        transform.i16[0] -= 64 * MULTIPLICITY;
        // finish the decoding
        message_array[i] = HQC_R4_128_AVX2_find_peaks(&transform);
    }
}


void HQC_R4_128_AVX2_reed_muller_decode_single(uint8_t *msg, const uint8_t *cdw) {
    __m256i expanded[8];
    __m256i transform[8];
    size_t i = 0;
    // collect the codewords
    HQC_R4_128_AVX2_expand_and_sum(expanded, (uint64_t *)&cdw[16 * i * MULTIPLICITY]);
    // apply hadamard transform
    HQC_R4_128_AVX2_hadamard(expanded, transform);
    // fix the first entry to get the half Hadamard transform
    ...
    // finish the decoding
    msg[i] = HQC_R4_128_AVX2_find_peaks(transform);
}