...
static uint16_t HQC_R4_128_AVX2_gf_reduce(uint64_t x, size_t deg_x);
 ...
 */
void HQC_R4_128_AVX2_gf_generate(uint16_t *exp, uint16_t *log, const int16_t m) {
    uint16_t elt = 1;
 ...
 */
uint16_t HQC_R4_128_AVX2_gf_mul(uint16_t a, uint16_t b) {
    ...
    return HQC_R4_128_AVX2_gf_reduce(ab, 2*(PARAM_M-1));
}
 ...
 * 
 */
__m256i HQC_R4_128_AVX2_gf_mul_vect(__m256i a, __m256i b) {
 ...
 * @param[in] a Element of GF(2^GF_M)
 */
uint16_t HQC_R4_128_AVX2_gf_square(uint16_t a) {
    uint32_t b = a;
    uint32_t s = b & 1;     
    ...
    return HQC_R4_128_AVX2_gf_reduce(s, 2 * (PARAM_M - 1));
}
 ...
 * @param[in] a Element of GF(2^GF_M)
 */
uint16_t HQC_R4_128_AVX2_gf_inverse(uint16_t a) {
    uint16_t inv = a;
    uint16_t tmp1, tmp2;

    inv = HQC_R4_128_AVX2_gf_square(a); /* a^2 */
    tmp1 = HQC_R4_128_AVX2_gf_mul(inv, a); /* a^3 */
    inv = HQC_R4_128_AVX2_gf_square(inv); /* a^4 */
    tmp2 = HQC_R4_128_AVX2_gf_mul(inv, tmp1); /* a^7 */
    tmp1 = HQC_R4_128_AVX2_gf_mul(inv, tmp2); /* a^11 */
    inv = HQC_R4_128_AVX2_gf_mul(tmp1, inv); /* a^15 */
    inv = HQC_R4_128_AVX2_gf_square(inv); /* a^30 */
    inv = HQC_R4_128_AVX2_gf_square(inv); /* a^60 */
    inv = HQC_R4_128_AVX2_gf_square(inv); /* a^120 */
    inv = HQC_R4_128_AVX2_gf_mul(inv, tmp2); /* a^127 */
    inv = HQC_R4_128_AVX2_gf_square(inv); /* a^254 */
    return inv;
}
 ...
 */
uint16_t HQC_R4_128_AVX2_gf_mod(uint16_t i) {
