...
thread_local shake256incctx HQC_R4_192_AVX2_shake_prng_state;
 ...
 */
void HQC_R4_192_AVX2_shake_prng_init(uint8_t *entropy_input, uint8_t *personalization_string, uint32_t enlen, uint32_t perlen) {
    uint8_t domain = PRNG_DOMAIN;
    HQC_R4_192_AVX2_shake256_inc_init(&HQC_R4_192_AVX2_shake_prng_state);
    HQC_R4_192_AVX2_shake256_inc_absorb(&HQC_R4_192_AVX2_shake_prng_state, entropy_input, enlen);
    HQC_R4_192_AVX2_shake256_inc_absorb(&HQC_R4_192_AVX2_shake_prng_state, personalization_string, perlen);
    HQC_R4_192_AVX2_shake256_inc_absorb(&HQC_R4_192_AVX2_shake_prng_state, &domain, 1);
    HQC_R4_192_AVX2_shake256_inc_finalize(&HQC_R4_192_AVX2_shake_prng_state);
}
 ...
 */
void HQC_R4_192_AVX2_shake_prng(uint8_t *output, uint32_t outlen) {
    HQC_R4_192_AVX2_shake256_inc_squeeze(output, outlen, &HQC_R4_192_AVX2_shake_prng_state);
}
 ...
 */
void HQC_R4_192_AVX2_seedexpander_init(seedexpander_state *state, const uint8_t *seed, uint32_t seedlen) {
    uint8_t domain = SEEDEXPANDER_DOMAIN;
    HQC_R4_192_AVX2_shake256_inc_init(state);
    HQC_R4_192_AVX2_shake256_inc_absorb(state, seed, seedlen);
 ...
 */
void HQC_R4_192_AVX2_seedexpander(seedexpander_state *state, uint8_t *output, uint32_t outlen) {
    const uint8_t bsize = sizeof(uint64_t);
    const uint8_t remainder = outlen % bsize;
    uint8_t tmp[sizeof(uint64_t)];
    HQC_R4_192_AVX2_shake256_inc_squeeze(output, outlen - remainder, state);
    if (remainder != 0) {
      HQC_R4_192_AVX2_shake256_inc_squeeze(tmp, bsize, state);
