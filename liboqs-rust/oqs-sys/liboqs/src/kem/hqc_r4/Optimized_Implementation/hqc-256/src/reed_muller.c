...
static void expand_and_sum(expandedCodeword *dst, codeword src[]);
 ...
 */
void HQC_R4_256_AVX2_reed_muller_encode(uint64_t *cdw, const uint64_t *msg) {
 ...
 */
void HQC_R4_256_AVX2_reed_muller_decode(uint64_t *msg, const uint64_t *cdw) {
    uint8_t *message_array = (uint8_t *) msg;
    codeword *codeArray = (codeword *) cdw;
...
void HQC_R4_256_AVX2_reed_muller_encode_single(uint8_t *cdw, const uint8_t *msg) {
    size_t i = 0;
    // encode first word
...
void HQC_R4_256_AVX2_reed_muller_decode_single(uint8_t *msg, const uint8_t *cdw) {
    __m256i expanded[8];
    __m256i transform[8];
    size_t i = 0;
    // collect the codewords
